...
class GraphProperties:
    ...
    def __post_init__(self):
        ...
        self.edge_ismassive: list[bool] = [mass > TOLERANCE for mass in self.edge_masses]
...
class Parameterisation(ABC):
    ...
    def __init__(self, 
        ...
        self.layer_continuous_dim_consumed = self.layer_continuous_dim_in - self.layer_continuous_dim_out
        ...
        self.chain_continuous_dim_out = self.chain_continuous_dim_in - self.layer_continuous_dim_consumed
        self.chain_discrete_dims = self.get_chain_discrete_dims()

    @abstractmethod
    def parameterise(self, input: LayerOutput,
                     ) -> LayerOutput:
        """
        Args:
            SamplerOutput: Output of the previous Parameterisation layer (or sampler)
        Returns:
            SamplerOutput: Parameterisation and data to be passed along the chain
        Raises:
            ValueError
        """
        pass
    ...
    def _pick_layer_input(self, input: LayerOutput
        ...
        Args:
            input: SamplerOutput of the previous layer
        Returns:
        ...
        if self.is_first_layer:
            jacobians = torch.ones(n_points)
            x_all = input.x_all
        else:
            jacobians, x_all = input.x_all.tensor_split([1], axis=1)
        
        n_points, n_dim = x_all.shape
        ...
        continuous, discrete = x_all.tensor_split([self.chain_continuous_dim_in], axis=1)
    ...
    def _to_chain_output(self, layer_input: LayerOutput, param_output: LayerOutput) -> LayerOutput:
        """
        Returns the output of the current layer in a form that respects the structure of the chain.
        
        Args:
            layer_input: SamplerOutput of the previous layer
            param_output: SamplerOutput generated by the parameterisation step of the current layer
        Returns:
            Jacobians: 1-dim torch.Tensor
            Continuous Samples: torch.Tensor of shape(n_samples, self.continuous_dim)
            Discrete Samples: torch.Tensor of shape(n_samples, <=len(self.discrete_dims))
        """
        timing = layer_input.timing.update(param_output.timing)
        discrete_in = layer_input.x_all[:, self.chain_continuous_dim_in:]
        discrete_param = param_output.x_all[:, self.chain_continuous_dim_out:]
...
class MomtropParameterisation(Parameterisation):
    def __init__(self, 
        ...
        assym_graph = momtrop.Graph(mt_edges, self.graph_properties.graph_external_vertices)
        momentum_shifts = [momtrop.Vector(*shift) for shift 
                           in self.graph_properties.edge_momentum_shifts]
        self.momtrop_edge_data = momtrop.EdgeData(self.graph_properties.edge_masses, momentum_shifts)
        self.momtrop_sampler = momtrop.Sampler(assym_graph, self.graph_properties.graph_signature)
        self.momtrop_sampler_settings = momtrop.Settings(False, False)
    
    def parameterise(self, input: SamplerOutput) -> Tuple[ParameterisationOutput, SamplerOutput]:
    ...
    def _get_graph_from_edges_removed(self, edges_removed: List[int] | None = None) -> List[int]:
        ...
        """
        full_graph = list(range(self.discrete_dims[0]))
